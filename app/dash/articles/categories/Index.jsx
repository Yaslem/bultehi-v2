...
export default function Index({ categories, status, message }) {
    ...
    const handelCreateCategory =  async (e) => {
        ...
        if(validated.success) {
            setError({})
            const { message, status, data } = await createCategory({name, slug, type: "ARTICLE", description: desc})
            if(status === "success"){
    ...
    const handelEditCategory = async (e) => {
        ...
        if(validated.success) {
            setError({})
            const { message, status, data } = await updateCategory({id: currentId, name, slug, type: "ARTICLE", description: desc})
            if(status === "success"){
                formRef.current.reset()
                setShowAdd(false)
                setIsEdit(false)
