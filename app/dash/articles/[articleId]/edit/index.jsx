...
import {createArticle, updateArticle, uploadImage} from "../../../../../controllers/Article";
...
export default function Edit({article, categoriesProps, tagsProps}) {
    ...
    article.tags.map(tagId => tagsOnArticle.push(tagId.tagId))

    const [catId, setCatId] = useState(article.categoryId)
    const [tagsIds, setTagsIds] = useState(tagsOnArticle)
    const [statusArticle, setStatus] = useState(article.status)
    const [commentStatus, setCommentStatus] = useState(article.commentStatus)
    const [image, setImage] = useState(article.image)
    const [newImage, setNewImage] = useState(article.image === null ? undefined : article.image)
    const [imageUrl, setImageUrl] = useState(`${process.env.BASE_URL}/uploads/articles/${article.image}`)
    ...
    const handelAddArticle = async () => {
        ...
        const validated =  AddArticleSchema.safeParse({ title, summary, image, status: statusArticle, body, comment: commentStatus, category: String(catId)  })
        if(validated.success) {
            setError({})
            const formData = new FormData()
            formData.append("image", newImage)
            const { message, status, data } = await updateArticle({
                id: article.id, title, body, category: catId, image, tags: tagsIds, commentStatus, status: statusArticle, summary, formData
            })
            if(status === "success"){
                setIsLoading(false)
