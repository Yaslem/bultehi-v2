...
import CardSide from "../../add/components/CardSide";
import Input from "../../../../components/Input";
import {IoClose, IoImage} from "react-icons/io5";
import classNames from "classnames";
import Section from "../../../components/Section";
import {DEFAULT_IMAGE_FOR_GRANT} from "../../../../../helpers/Global";
import {updateGrant} from "../../../../../controllers/Grant";
...
export default function Edit(
    ...
    const [college, setCollege] = useState(grant.collegeId)
    const [phase, setPhase] = useState(grant.phaseId)
    const [specialization, setSpecialization] = useState(grant.specializationId)
    const [country, setCountry] = useState(grant.country)
    const [image, setImage] = useState(grant.image)
    const [title, setTitle] = useState(grant.title)
    const [isFree, setIsFree] = useState(grant.isFree)
    const [price, setPrice] = useState(grant.price)
    const [body, setBody] = useState(grant.description)
    const [newImage, setNewImage] = useState(grant.image === null ? undefined : grant.image)
    const [imageUrl, setImageUrl] = useState(`${process.env.BASE_URL}/uploads/grants/${grant.image}`)
    ...
    const handelUpdateGrant = async () => {
        ...
        if (validated.success) {
            ...
            if (Object.keys(errorSome).length === 0) {
                ...
                const { message, status, data } = await updateGrant(formData)
                if(status === "success"){
                    setIsLoading(false)
            ...
            <section className={"flex gap-2 w-full"}>
                <div className={"flex-grow rounded-lg flex flex-col gap-3"}>
                    ...
                    <div className={"border rounded-lg flex flex-col gap-2"}>
                        ...
                        {
                            error?.description &&
                            <div className={"mx-2 mb-2"}><Error message={error?.description._errors.join()}/></div>
                        }
                    </div>
                </div>
                    ...
                    <CardSide title={"المعلومات"} showTab={showTab} onClick={() => setShowTab(0)} number={0}>
                        {
                            ...
                            <>
                                ...
                                />
                                <Switch title={"منحة مجانية"} isTitle={true} name={"isFree"}
                                        checked={isFree} onChange={async (e) => setIsFree(e.target.checked)}/>
                                {
                                    !isFree &&
                                    <div className={"flex flex-col gap-2"}>
                                        <Input
                                        ...
                                        <ul className={"flex max-h-[190px] overflow-y-auto flex-col gap-3"}>
                                            {
                                                colleges.data.map((collegeProp, index) =>
                                                    <li key={index}
                                                        className={"text-xs bg-stone-50 rounded-lg p-2 flex items-center justify-between gap-2 hover:text-indigo-500 cursor-pointer"}>
                                                        <label htmlFor={collegeProp.id}
                                                               className={"flex-grow cursor-pointer"}>{collegeProp.name}</label>
                                                        <input
                                                            id={collegeProp.id}
                                                            name={"college"}
                                        ...
                                        <ul className={"flex max-h-[190px] overflow-y-auto flex-col gap-3"}>
                                            {
                                                specializations.data.map((specializationProp, index) =>
                                                    <li key={index}
                                                        className={"text-xs bg-stone-50 rounded-lg p-2 flex items-center justify-between gap-2 hover:text-indigo-500 cursor-pointer"}>
                                                        <label htmlFor={specializationProp.id}
                                                               className={"flex-grow cursor-pointer"}>{specializationProp.name}</label>
                                                        <input
                            ...
                            <div className={"relative"}>
                                {
                                    ...
                                    <>
                                        <label htmlFor={"image"}
                                               className={"cursor-pointer flex items-center justify-center p-2 w-full h-28 border border-dashed rounded-lg hover:border-indigo-600"}>
                                ...
                                <div className={classNames({
                                    ...
                                    "hidden": newImage === undefined
                                })}>
                                    <span
                                        className={"w-8 p-1 h-8 rounded-lg bg-white cursor-pointer border hover:border-red-600 absolute top-1.5 left-1.5 flex items-center justify-center"}>
                                        <IoClose
