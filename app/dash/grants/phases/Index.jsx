...
export default function Index({ categories, status, message }) {
    ...
    const handelCreateCategory =  async (e) => {
        ...
        if(validated.success) {
            setError({})
            const { message, status, data } = await createCategory({name, slug, type: "PHASE", description: desc})
            if(status === "success"){
    ...
    const handelEditCategory = async (e) => {
        ...
        if(validated.success) {
            setError({})
            const { message, status, data } = await updateCategory({id: currentId, name, slug, type: "PHASE", description: desc})
            if(status === "success"){
                formRef.current.reset()
                setShowAdd(false)
                setIsEdit(false)
    ...
    return (
        <Section>
            {
                showAdd && <CardAdd onClick={() => setShowAdd(false)} title={isEdit ? "تعديل مرحلة" : "إضافة مرحلة"}>
                    <form ref={formRef} onSubmit={isEdit ? handelEditCategory : handelCreateCategory} className={"flex flex-col gap-3"}>
                        <Input onChange={(e) => setName(e.target.value)} isError={error?.name} defaultValue={name} type={"text"} name={"name"} label={"الاسم"} placeholder={"اكتب اسم المرحلة"} />
                        {
                        ...
                        <TextArea onChange={(e) => setDesc(e.target.value)} defaultValue={desc} name={"des"} label={"الوصف"} placeholder={"اكتب وصف المرحلة"} />
                        <Button title={isEdit ? "تعديل المرحلة" : "إضافة مرحلة"} isLoading={isLoading} />
                    </form>
                </CardAdd>
            }
            <div className={"flex items-center justify-between gap-4"}>
                <Title title={"المراحل"} />
                <Button onClick={() => setShowAdd(true)} title={"إضافة مرحلة"} />
            </div>
            {
                status === "success" &&
                <Table th={["الاسم", "الاسم اللطيف", "الوصف", "المنح", "خيارات"]}>
                    {
                        categories.map((category, index) =>
                            <tr key={index} className={"p-2 even:border-y border-dashed"}>
                                ...
                                <Td value={category.grantsByPhase.length}/>
                                <Td value={
            ...
            { status === "error" && <Nothing title={"عفوا 😔"} desc={"المعذرة منك، لم نتمكن من العثور على مراحل، يرجى إضافة مراحل جديدة لعرضها. (:"} /> }
        </Section>
    )
}