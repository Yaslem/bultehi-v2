...
export default function Index({ tags, status, message }) {
    ...
    const handelCreateTag =  async (e) => {
        ...
        if(validated.success) {
            setError({})
            const { message, status, data } = await createTag({name, slug, type: "BOOK", description: desc})
            if(status === "success"){
    ...
    const handelEditTag = async (e) => {
        ...
        if(validated.success) {
            setError({})
            const { message, status, data } = await updateTag({id: currentId, name, slug, type: "BOOK", description: desc})
            if(status === "success"){
                formRef.current.reset()
                setShowAdd(false)
                setIsEdit(false)
