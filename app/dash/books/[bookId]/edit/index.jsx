...
export default function Edit(
    ...
    book.tags.map(tagId => tagsOnBook.push(tagId.tagId))
    book.authors.map(authorId => authorsOnBook.push(authorId.authorId))

    const [catId, setCatId] = useState(book.categoryId)
    const [authorsIds, setAuthorsIds] = useState(authorsOnBook)
    const [publisherId, setPublisherId] = useState(book.publisherId)
    const [tagsIds, setTagsIds] = useState(tagsOnBook)
    const [image, setImage] = useState(book.image)
    const [newImage, setNewImage] = useState(book.image === null ? undefined : book.image)
    const [imageUrl, setImageUrl] = useState(`${process.env.BASE_URL}/uploads/books/${book.image}`)
    const [title, setTitle] = useState(book.title)
    const [isFree, setIsFree] = useState(book.isFree)
    const [price, setPrice] = useState(book.price === null ? undefined : book.price)
    const [numberOfPages, setNumberOfPages] = useState(book.numberOfPages)
    const [numberOfCopies, setNumberOfCopies] = useState(book.numberOfCopies === null ? undefined : book.numberOfCopies)
    const [publishYear, setPublishYear] = useState(new Date(book.publishYear))
    const [body, setBody] = useState(book.description)
    ...
    const handelAddBook = async () => {
        ...
        if (validated.success) {
            ...
            if (Object.keys(errorSome).length === 0) {
                ...
                const { message, status, data } = await updateBook({id: book.id, title, numberOfPages, numberOfCopies, publishYear, price, isFree, description: body, categoryId: catId, tags: tagsIds, publisherId, authors: authorsIds, formData })
                if(status === "success"){
                    setIsLoading(false)
                    ...
                    <CardSide title={"صورة الكتاب"} showTab={showTab} onClick={() => setShowTab(5)} number={5}>
                        {
                            ...
                            <div className={"relative"}>
                                ...
                                <div className={classNames({
                                    ...
                                    <img src={imageUrl || image} className={"w-full object-cover h-full"} />
                                </div>
                            </div>
                        }
                        {
